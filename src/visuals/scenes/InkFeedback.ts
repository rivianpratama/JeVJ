 ...
 * for about a frame, and marbling for a minute.
 *
...
export class InkFeedback implements Scene {
  ...
  private allocate(w: number, h: number): void {
    ...
    // A resize throws the ink away: the densities are in uv space and nothing
    // sensible can be carried across a change of aspect.
    this.ping?.dispose();
    this.pong?.dispose();
    this.color?.dispose();
    this.ping = densityTarget(width, height, this.type);
    this.pong = densityTarget(width, height, this.type);
    this.color = densityTarget(width, height, this.type);

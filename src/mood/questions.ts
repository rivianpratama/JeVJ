 ...
 * this file (the HUD and the visuals want to know the label sets), and the SDK
 * refuses to load in a browser at all; the question shapes below are plain
 * objects that match the SDK's `Questions` structurally, so the server can hand
 * them straight to `systemOne`.
 *
 * **All eighteen, every call.** v1 chose a subset per call — ten core
 * questions always, four nouls on alternate calls, four predictive ones only
 * into a build — because it asked several times a minute while a track played
 * and the questions are nearly the whole cost of a request. v2 asks once per
 * segment, before a note sounds, so the saving is a rounding error against a
 * track's total and the selection was one more thing that could be wrong about
 * a call nobody could replay. It is gone, along with the `ask`/`prev` protocol
 * that carried it (see `server/moodHandler.ts`).
 */
